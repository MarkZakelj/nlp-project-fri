...
def read_data(path):
    ...
    if os.path.isfile(path):
        ...
        with os.scandir(path) as it:
            groups = []
            for entry in it:
                if entry.is_file() and not entry.name.startswith('.') and os.path.splitext(entry.name)[1].lower() in [
                    '.tsv', '.csv']:
                    try:
...
def export_REL_REL_FRAME(datalines, groups, outfile):
    ...
    for g in groups:
        ...
        for relation in relations:
            ...
            row = {'REL_LABEL': re.sub('\[[0-9]+\]$', '', relation).replace('\\', ''),
                   # strip ending number in brackets and \
...
def make_network(groups):
    ...
    for g in groups:
        ...
        for val in list(g[colid].value_counts().index):
            rows = g.loc[g[colid] == val]
            string = ' '.join([x.strip() for x in rows[COLNAME2POS['TOKEN']].values.tolist() if x.strip()]).replace(
                ' ,', ',').replace('( ', '(').replace(' )', ')')
            if string.isupper():
                string = string.lower()
            string = uncapitalize(string)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='A converter from WebAnno tsv to csv (multiple values per cell may occur)',
    ...
    parser.add_argument('-n', '--network', action='store_true',
                        help="Also export a network created from parsed .tsv/.csv files")
    args = parser.parse_args(['EN/defs-en-19.tsv'])
    args = parser.parse_args(['EN'])
    #
    ...
    # df_full.columns = ['word', 'hierarchical', 'non_hierarchical', 'sentence_number']
    for col_num in [5,6]:
        df_full[col_num] = df_full[col_num].apply(lambda x: x[:x.find('[')].replace('\\', '') if not type(x) == float else x)

    df_full = df_full.reset_index(drop=True).iloc[:, [3, 0, 1, 2]]
    df_full.columns = ['sentence', 'word', 'hierarchical', 'non-hierarchical']
    print(df_full[df_full['word'] == 'eddies'])

    df_full.to_csv('full_data.csv', index=False)

    # if datalines:
    #     print('Exporting...')
    #     export_TERM_CAT_GEN_SENT_REL(datalines, os.path.join('{}{}{}'.format(base, sep, TERM_CAT_GEN_SENT_REL_file)))
    #     export_TERM_CATEGORY(datalines, os.path.join('{}{}{}'.format(base, sep, TERM_CATEGORY_file)))
